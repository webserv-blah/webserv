...
EnumSesStatus EventHandler::recvRequest(ClientSession &curSession) {
	std::clog << "Start RecvReq" << std::endl;
	std::string buffer;
	...
	if (res == -1) {
		...
		||  errno == EINTR) {			//errno)인터럽트, 시스템 호출 중 시그널에 의해 중단되어 호출이 완료되지 않은 경우 {
			std::clog << "READ_CONTINUE bcuz of errno " << strerror(errno) << std::endl;
			return READ_CONTINUE;
		}
		return CONNECTION_CLOSED;	//errno)그 외, 통신을 할 수 없는 상태이거나, 메모리 문제로 치명적인 경우
	} else if (res == 0) {			//클라이언트 측에서 연결을 종료한 경우
		std::clog << "[recvRequest]client Closed connection" << std::endl;
		...
		std::clog << "PRINT REQUEST MSG" << std::endl;
		curSession.getReqMsg().printResult();
		...
		if (curSession.getErrorStatusCode() != NONE_STATUS_CODE) {
			...
			curSession.setConfig(globalConfig.findRequestConfig(curSession.getListenFd(), "", ""));
