...
EnumStatusCode RequestParser::cleanUpChunkedBody(std::string &readBuffer, RequestMessage &reqMsg) {
	...
	size_t cursorBack = 0;
	std::clog << "\033[32;7mSTARTING CHUNK:\033[0m" << std::endl;
	...
	while (1) {
		...
		std::cout << "청크 사이즈: " << chunkSize << ";\n";
		std::cout << "청크전 남은양: " << readBuffer.size() - cursorBack << ";\n";
		if (chunkSize + 2 > readBuffer.size() - cursorBack) {
			readBuffer.erase(0, cursorFront);
			return NONE_STATUS_CODE;
		}
		
		std::cout << "QQQ: " << readBuffer.find(CRLF, cursorBack + chunkSize) << ";\n";
